...
public class Fitness  extends Mapper  <LongWritable, Text, DoubleWritable, Text> {	
	...
	public void map(LongWritable key, Text value, Context context)
		...
		String decode=decodeChromo(chomo);
		int target=Integer.parseInt(context.getConfiguration().get("target"));
		double fit=0;
		if(target!=compute(decode)&&compute(decode)!=0)
			fit=1.0/Math.abs(compute(decode)-target);
		context.write(new DoubleWritable(fit), new Text(chomo+","+decode));
	}
